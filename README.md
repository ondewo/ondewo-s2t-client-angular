...
The hand-written surface is `src/lib/auth/` (the Keycloak bearer-auth layer) and `examples/`.
Everything else — `api/`, `fesm2022/`, `index.d.ts`, `public-api.ts` — is generated by the
proto-compiler and is neither linted for logic nor measured.
...
`jest.config.js` gates both hand-written roots at 100% statements / branches / functions / lines.
...
`src/lib/auth/` provides `provideOndewoS2tAuth`, `KeycloakTokenProvider`, `AuthGrpcInterceptor` and
...
> :warning: The generated `public-api.ts` does not re-export the `src/lib/auth` barrel yet, so these
> symbols are not reachable from the published npm bundle. Emitting that re-export needs a `make
> build` with `ondewo-proto-compiler` >= 5.13.0 (the pin is already at 5.14.0; the stubs are not
> regenerated yet).
