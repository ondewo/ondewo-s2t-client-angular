...
release: ## Create Github and NPM Release
	...
	git add api
	git add fesm2022
	@# Regenerated by every build and carrying the auth surface into the published typings:
	@# staging them keeps the release tag describing the same package npm receives.
...
check_out_correct_submodule_versions: ## Fetches all Submodules and checks out specified branch
	...
	@# release. They have also held a real branch during feature work (S2T_API_GIT_BRANCH=master,
	@# =OND211-2418-add-keycloak-for-2-fa), where the pull is what advances the checkout past what
	@# `fetch --all` left behind - so pull only when HEAD is on a branch, and stay non-fatal.
	-if git -C ${S2T_APIS_DIR} symbolic-ref -q HEAD >/dev/null; then git -C ${S2T_APIS_DIR} pull; fi
